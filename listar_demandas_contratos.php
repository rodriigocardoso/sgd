...
$qr_demanda = $db->query("SELECT * FROM demanda INNER JOIN aquisicao on demanda.cod_demanda = aquisicao.cod_demanda_fk INNER JOIN projeto on demanda.cod_projeto_fk = projeto.cod_projeto INNER JOIN area on demanda.cod_area_fk = area.cod_area INNER JOIN diretoria on area.cod_diretoria_fk = diretoria.cod_diretoria INNER JOIN contrato on demanda.cod_demanda = contrato.cod_demanda_fk ") ;
?>
                    ...
                    <div class="col-md-12">
                        <div class="card">
                            <div class="card-header">
                                <strong class="card-title">Lista de Demandas Contratos e Parcerias</strong>
                            </div>
                            <div class="card-body">
...
<td><?php 
if (!empty($linhas['dt_assinatura']) AND $linhas['dt_assinatura'] != '0000-00-00'){
echo date('d/m/Y' , strtotime ($linhas['dt_assinatura']));

} ?></td>
<td><?php 
if (!empty($linhas['vencimento']) AND $linhas['vencimento'] != '0000-00-00'){
echo date('d/m/Y' , strtotime ($linhas['vencimento']));

} ?></td>
<td><?php echo $linhas['fase_demanda'] ?></td>

<td><a class="nav-link" href=editar_contratos.php?cod_demanda=<?php echo $linhas['cod_demanda'] ?>"><i class="fa fa-edit"></i> Editar</a></td>
<td><a class="nav-link" href=completo_contratos.php?cod_demanda=<?php echo $linhas['cod_demanda'] ?>"><i class="fa fa-clone"></i> Completo</a></td>
...
<?php } ?>
 ...
 <div>
<tr><a href="dompdf/contratos_pdf.php" type="button" class="btn btn-danger btn-sm" > 
             <i class="fa fa-file-pdf-o">EXPORTAR PDF </i></a> </tr>
             
<tr><a href="contratos_excel.php" type="button" class="btn btn-btn btn-success btn-sm" > 
