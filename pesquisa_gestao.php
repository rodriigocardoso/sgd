...
?>
 ...
 <a class="nav-link" href="#"><i class="fa fa-user"></i><?php echo  $_SESSION['sigla_diretoria']; ?></a>
        ...
        <div class="breadcrumbs">
			...
			<center>
                <form action="pesquisa_gestao.php" method="POST" autocomplete="off" >
<div>
...
$qr_demanda = $db->query("SELECT * FROM demanda INNER JOIN aquisicao on demanda.cod_demanda = aquisicao.cod_demanda_fk INNER JOIN projeto on demanda.cod_projeto_fk = projeto.cod_projeto INNER JOIN area on demanda.cod_area_fk = area.cod_area INNER JOIN diretoria on area.cod_diretoria_fk = diretoria.cod_diretoria INNER JOIN gestao on demanda.cod_gestao_fk = gestao.cod_gestao INNER JOIN contrato on demanda.cod_demanda = contrato.cod_demanda_fk WHERE demanda.objeto LIKE '%$pesquisar%'");
...
elseif ($count == 0){
echo "Registro não encontrado! <br> Para cadastrar uma Nova Demanda entre em contato com área responsável.</a>"."</center>";
}
else { ?>
...
<td><a class="nav-link" href=editar_gestao.php?cod_demanda=<?php echo $linhas['cod_demanda'] ?>"><i class="fa fa-edit"></i> Editar</a></td>
<td><a class="nav-link" href=completo_gestao.php?cod_demanda=<?php echo $linhas['cod_demanda'] ?>"><i class="fa fa-clone"></i> Completo</a></td>
